  ...
  const handleButtonClick = (label) => {
    ...
    if (label === '=') {
      try {
        ...
        setHistory([...history, `${displayValue} = ${evalResult}`]);
      ...
      <div className="buttons">
        ...
        {['2nd', 'x²', 'x³', 'xʸ', 'eˣ', '10ˣ', '7', '8', '9', '×'].map((label) => (
          <Button
            ...
            className={(label === '×' ? 'orange' : '') + (['7', '8', '9'].includes(label) ? ' grey' : '')}
          />
        ))}
        {['1/x', '²√x', '³√x', 'ʸ√x', 'ln', 'log₁₀', '4', '5', '6', '-'].map((label) => (
          <Button
            ...
            label={label}
            onClick={handleButtonClick}
            className={(label === '-' ? 'orange' : '') + (['4', '5', '6'].includes(label) ? ' grey' : '')}
          />
        ))}
        {['x!', 'sin', 'cos', 'tan', 'e', 'EE', '1', '2', '3', '+'].map((label) => (
          <Button
            ...
            label={label}
            onClick={handleButtonClick}
            className={(label === '+' ? 'orange' : '') + (['1', '2', '3'].includes(label) ? ' grey' : '')}
          />
        ))}
        {['Rad', 'sinh', 'cosh', 'tanh', 'π', 'Rand', '0',' ' , '.', '='].map((label) => (
          <Button
            ...
            label={label}
            onClick={handleButtonClick}
            className={`${label === '=' ? 'orange' : ''} ${['7', '8', '9', '0', ' ' , '.'].includes(label) ? ' grey' : ''} ${label === '0' ? 'zero' : ''}`}
          />
        ))}
